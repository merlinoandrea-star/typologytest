...
def calculate_type(responses: dict) -> dict:
    ...
    # Funzioni ordinate per punteggio
    funcs = sorted([("T",norm_T),("F",norm_F),("S",norm_S),("N",norm_N)], key=lambda x: -x[1])
    dominant = funcs[0][0]
    
    # Ausiliaria: opposta categoria (razionale T/F vs irrazionale S/N)
    rational = {"T","F"}
    irrational = {"S","N"}
    ...
    # Codice tipo (compatibilità con sistema esistente)
    type_code = f"{orientation}-{dominant}{auxiliary}"

    # Nome nel nuovo formato
...
def chat(data: ChatMessage):
    ...
    # Determina quale domanda presentare
...
Sii concisa: max 3 righe in totale."""
        ...
        # Tutte le domande risposte — calcola il profilo
...
Poi spiega brevemente cosa significa ciascuna funzione per questa persona specifica.
...
# ─────────────────────────────────────────────────────────────────────────────
# ENDPOINT: calcola risultato diretto (modalità form classica)
# ─────────────────────────────────────────────────────────────────────────────
@app.post("/api/submit")
...
@app.get("/")
def root():
    return {"status": "ok", "message": "Typology API v2"}
